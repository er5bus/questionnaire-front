...
export default (state = { items: [], item: {}, allCompanies: [], page: 0, isLoading: false, searchTerm: "", hasMore: true, error: null }, action) => {
  ...
  switch (type) {
    ...
    case ACTIONS.EDIT_MANAGER_INVITATION_SUCCEDED : {
    ...
    case ACTIONS.FETCH_MANAGER_INVITATIONS_SUCCEDED : {
    ...
    case ACTIONS.FETCH_MANAGER_INVITATIONS_FAILED : {
      return { ...state, isLoading: false, hasMore: false, page: 1, error: payload }
    }

    case ACTIONS.FETCH_MANAGER_INVITATION_INIT : {
      return { ...state, isLoading: true, item: null, error: null }
    }
    case ACTIONS.FETCH_MANAGER_INVITATION_SUCCEDED : {
      return { ...state, item: payload, isLoading: false, error: null }
    }
    case ACTIONS.FETCH_MANAGER_INVITATION_FAILED : {
      return { ...state, isLoading: false, error: payload }
    }

    case ACTIONS.DELETE_MANAGER_INVITATION_INIT : {
      return { ...state, isLoading: true, error: null }
    }
    case ACTIONS.DELETE_MANAGER_INVITATION_SUCCEDED : {
      return { ...state, items: state.items.filter( item => item.id !== payload.id ), isLoading: false, error: null }
    }
    case ACTIONS.DELETE_MANAGER_INVITATION_FAILED : {
      return { ...state, isLoading: false, error: null }
    }

    case ACTIONS.FILTER_MANAGER_INVITATIONS: {
